...
/-!
# Winding numbers from the covering map `exp`

This file is a thin wrapper around Mathlib's covering-space API for
`isCoveringMap_exp`. It does three things:

* packages homotopies through loops on `I`,
* defines winding numbers of loops in `ℂ \ {0}` via lifts through `exp`,
...
@[simp] theorem windingNumber_refl (z : Cstar) :
  ...
  let Γ : C(I, ℂ) := ContinuousMap.const _ (log z)
  have hlift : ∀ t, exp (Γ t) = ((Path.refl z) t : ℂ) := by
    intro t
    simpa [Γ] using exp_log z.property
  have hq : (Γ 1 - Γ 0) / (2 * π * 𝓲) = (Path.refl z).windingNumber :=
